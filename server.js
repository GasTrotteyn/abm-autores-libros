...
server.post("/autores", (req, res) => {
    ...
    let autor = autores.find(
        (element) =>
            element.nombre === nuevoAutor.nombre &&
            element.apellido === nuevoAutor.apellido &&
            element.id === nuevoAutor.id
    );
    if (autor) {
        res.status(409).send("ese autor o ese id ya existen");
    } else {
...
server.post("/autores/:id/libros", estaAutor, (req, res) => {
    ...
    let libro = req.autorEncontrado.libros.find(
        (element) => element.id === libroInsertado.id
    );
    if (!libro) {
...
server.put("/autores/:id/libros/:idLibro", estaAutor, estaLibro, (req, res) => {
    ...
    res.status(200).send(
        `se ha remplazado ${JSON.stringify(
            req.libroEncontrado.titulo
        )} por ${JSON.stringify(req.body.titulo)}`
    );
});
